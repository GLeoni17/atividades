<?php
    ...
    while($row = mysqli_fetch_assoc($res)){
        ...
        echo "<h2><b onmouseover=\"tts('".$row["nome"]."')\">".$row["nome"]."</b>"; 
        if($permissao==4|| $permissao==3 && $id_usuario == $cod_campeonato){ // So pode alterar o dado se for o dono do campeonato
            echo "<button class='alterar_campeonato' value='$id' data-toggle='modal' data-target='#modal' onmouseover=\"tts('alterar campeonato')\">✏️</button> 
            <button class='remover_campeonato' value='$id' onmouseover=\"tts('remover campeonato')\">ˣ</button>";
        }
        echo "</h2><ul>";
